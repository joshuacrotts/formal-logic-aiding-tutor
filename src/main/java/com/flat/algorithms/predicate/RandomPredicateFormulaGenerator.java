 ...
 *
 */
public final class RandomPredicateFormulaGenerator {
    ...
    private void genPredicate(StringBuilder _sb) {
        ...
        if ((Math.random() < usedPredicateProbability && !this.predicatesNotUsed.isEmpty()) || this.predicatesUsed.isEmpty()) {
            p = this.predicatesNotUsed.remove(FLATUtils.randomInt(this.predicatesNotUsed.size() - 1));
            this.predicatesUsed.add(p);
        } else {
            p = this.predicatesUsed.get(FLATUtils.randomInt(this.predicatesUsed.size() - 1));
        }
    ...
    private Character getRandomVariable() {
        ...
        if ((Math.random() < usedVariableProbability && !this.variablesNotUsed.isEmpty()) || this.variablesUsed.isEmpty()) {
            v = this.variablesNotUsed.remove(FLATUtils.randomInt(this.variablesNotUsed.size() - 1));
            this.variablesUsed.add(v);
        } else {
            v = this.variablesUsed.get(FLATUtils.randomInt(this.variablesUsed.size() - 1));
        }
    ...
    private Character getRandomConstant() {
        ...
        if ((Math.random() < usedConstantProbability && !this.constantsNotUsed.isEmpty()) || this.constantsUsed.isEmpty()) {
            c = this.constantsNotUsed.remove(FLATUtils.randomInt(this.constantsNotUsed.size() - 1));
            this.constantsUsed.add(c);
        } else {
            c = this.constantsUsed.get(FLATUtils.randomInt(this.constantsUsed.size() - 1));
        }
    ...
    private String getRandomBinaryOp() {
        return this.binaryOperators.get(FLATUtils.randomInt(this.binaryOperators.size() - 1));
    }
}
